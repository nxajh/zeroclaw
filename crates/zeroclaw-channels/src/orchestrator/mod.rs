...
async fn process_channel_message(
    ...
    enum LlmExecutionResult {
        Completed(Result<Result<Vec<String>, anyhow::Error>, tokio::time::error::Elapsed>),
        Cancelled,
    }
        ...
        LlmExecutionResult::Completed(Ok(Ok(per_round_texts))) => {
            ...
            if let Some(channel) = target_channel.as_ref() {
                if let Some(ref draft_id) = draft_message_id {
                    // Draft-capable channels (Telegram/Discord/Matrix):
                    // finalize the draft with the full joined text.
                    if let Err(e) = channel
    ...
    match channel_id {
        ...
        "wechat" => {
            #[cfg(feature = "channel-wechat")]
            {
                ...
                Ok(Arc::new(WechatChannel::new(wc.clone())))
            }
...
fn collect_configured_channels(
    ...
    if let Some(ref wc) = config.channels.wechat {
        if wc.enabled {
            channels.push(ConfiguredChannel {
                display_name: "WeChat",
                channel: Arc::new(WechatChannel::new(wc.clone())),
            });
        } else {
            tracing::info!("WeChat channel configured but disabled");
        }
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        show_reasoning_content: config.agent.show_reasoning_content,
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    fn compact_sender_history_keeps_recent_truncated_messages() {
        ...
        let ctx = ChannelRuntimeContext {
            ...
            model_routes: Arc::new(HashMap::new()),
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
...

        {
            ...
            store.insert(
                ...
                RuntimeConfigState {
                    defaults: ChannelRuntimeDefaults {
                        route_key: "test-provider".to_string(),
...
This is an example JSON object for profile settings."#;
    ...
    async fn process_channel_message_applies_query_classification_route() {
        ...
        let model_routes: HashMap<String, String> = [(
            "vision".to_string(),
            "vision-provider/gpt-4-vision".to_string(),
        )]
        .into_iter()
        .collect();
    ...
    async fn process_channel_message_classification_priority_selects_highest() {
        ...
        let model_routes: HashMap<String, String> = [
            ("fast".to_string(), "fast-provider/fast-model".to_string()),
            ("code".to_string(), "code-provider/code-model".to_string()),
        ]
        .into_iter()
        .collect();
